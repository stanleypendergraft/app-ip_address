 ...
 * @return {object.<string, sting>} (returnObj) - An IPv4 address, IPv6 address.
 */
function getFirstIpAddress(cidrStr, callback) {
  ...
  if (!cidr.isValid()) {
    ...
    [ipv4Address] = cidr.toArray(options);
    returnObj.ipv4 = ipv4Address;
    returnObj.ipv6 = getIpv4MappedIpv6Address(ipv4Address);
  }
...
function main() {
  ...
  for (let i = 0; i < sampleCidrsLen; i++) {
    ...
    getFirstIpAddress(sampleCidrs[i], (data, error) => {
      ...
      console.log(`  Response returned from GET request: ${json}`);
    });
  }
